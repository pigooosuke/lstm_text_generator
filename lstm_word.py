...
def get_word_dict(contents):
    vocab = sorted(set(contents.split(" ")))
    print("total vocab:", len(vocab))
    print("total words:", len(contents.split(" ")))
    vocab_to_int = {c:i for i,c in enumerate(vocab)}
    int_to_vocab = dict(enumerate(vocab))
    encoded = np.array([vocab_to_int[c] for c in contents.split(" ")], dtype=np.int32)
...
def test(vocab, vocab_to_int, int_to_vocab):
    ...
    samp = sample(checkpoint, vocab, vocab_to_int, int_to_vocab, 300, lstm_size, len(vocab), prime="The")
    print(samp)
